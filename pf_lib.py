...
def main(file_name, selected_methods):
    ...
    # above parser
    select_method(file_name, motif_distribution,
                  exponential, self_power, threshold)
    return None


def select_method(file_name, motif_distribution, exponential, self_power, threshold):
    '''
    This fuction can let the user choose which method that want to
...
def run_pf_plot(raw_row, reduce_size, smooth_size):
    '''
    This fuction calls the find_peaks_and_or_valleys() to find the
    ...
    '''
    reduced_row, peaks, valleys, peak_properties, 
    valley_properties = find_peaks_and_or_valleys(
    ...
    print("---------------------------------------------------")
    return raw_row, educed_row, peaks, valleys, peaks_bp, valleys_bp
...
def _preprocess_mds_file(file_name):
    '''
    This fuction will read a motif distribution (MD) csv file. After
    ...
    return raw_data, exp_data, selfpower_data, threshold_data
...
def draft_categorize_msd_file_into_tf_regulation(mds_file_path, tf_regulation_file):
    ...
    ids, raw_data = preprocess_mds_file(mds_file_path)
    ...
    for i in range(0, len(raw_data)):
        ...
        reduced_row, peaks, valleys, peak_properties, valley_properties =
        find_peaks_and_or_valleys(
        ...
        tf_regulation_line = '{},{},{},{},{}'.format(
            peak_count, valley_count, id, sra, category
        )
