...
def reduce_data_for_peak_detection(raw_row,reduce_size,smooth_size):
...
def find_peaks_and_or_valleys(raw_row, reduce_size, smooth_size):
    """ Take raw data and call determine_scans() to scan for peaks and valley based on detection of continuous signal
    ...
    """
    reduced_row = reduce_data_for_peak_detection(raw_row, reduce_size, smooth_size)
    peak_scan, valley_scan = determine_scans(reduced_row)
...
def determine_scans(reduced_row):
